...
void main() {
    ...
    vec3 v = normalize(-relativeVertexPosition3);
    vec3 r = reflect(-l, n);
    ...
    float intensity=0.6*nlAngle+0.4*specAngle;
    if (intensity>0.95){
        intensity=1.1;
    ...
    mix(materialSpecularColor, maskLightColor, billardTexture.rgb)*lightColor * specular)*intensity;
